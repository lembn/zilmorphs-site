...
export const Card = observer(({ morph, owned }: { morph: ApiMorph; owned?: boolean }) => {
    ...
    async function transfer() {
        try {
            ...
            const result = await zilmorphsSdk
                .calls(addressbook.ZILMORPHS_ADDRESS)(Long.fromString("40000"))
    ...
    return (
        <>
            {show && (
                <Layer onEsc={() => setShow(false)} onClickOutside={() => setShow(false)} responsive={false} plain>
                    <Box
                        alignSelf="center"
                        justify="center"
                        height="medium"
                    ...
                    >
                        <Box>
                            ...
                            <Box>
                                <TextInput
                                    placeholder={"checksummed or bech32 address"}
                                    size="small"
                                    value={target}
            ...
            >
                ...
                <Box direction="row" fill pad="small">
                    <Box fill>
                        ...
                        <Text size="xsmall" style={{ fontWeight: "bold" }} color="neutral-3">{`Power: ${(
                            morph.data.stats.str +
                            morph.data.stats.agi +
                            morph.data.stats.int
                        ).toFixed(3)}`}</Text>
                        <Text size="xsmall" style={{ fontWeight: "bold" }}>{`Strength: ${morph.data.stats.str}`}</Text>
                        <Text size="xsmall" style={{ fontWeight: "bold" }}>{`Agility: ${morph.data.stats.agi}`}</Text>
                        <Text
                            size="xsmall"
                    ...
                    <Box fill>
                        ...
                        {morph.data.stats.special.length == 0 ? (
                            ...
                            morph.data.stats.special.map((s) => (
                                <Text key={s.desc} size="xsmall" style={{ fontWeight: "bold" }}>
                                    {`${s.desc}`}
                                </Text>
                            ))
                        )}
                ...
                {!hide && owned && (
                    <Box direction="row" gap="small">
                        ...
                        />
                        <Button label={"get png"} style={{ fontWeight: "bold" }} size="small" onClick={() => toPng()} />
                    </Box>
                )}
            </Box>
        </>
